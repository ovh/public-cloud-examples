...
resource "ovh_cloud_project" "hub" {
  ...
  plan {
    ...
    configuration {
      label = "vrack"
      value = ovh_vrack.hub_vrack.service_name
    }
  }
}

########################################################################################
#   Public Cloud Project: Spoke QA
########################################################################################

resource "ovh_cloud_project" "spoke_qa" {
  ovh_subsidiary = data.ovh_order_cart.mycart.ovh_subsidiary
  description    = "spokeqa${local.name_suffix}"
  ...
  plan {
    ...
    configuration {
      label = "vrack"
      value = ovh_vrack.spoke_qa_vrack.service_name
    }
  }
}
